...
eval_head(Ex, Inlist, 0, 8, Inlist) :- functor(Ex, P, _), my_call(Ex), member(P,[equal, min_list, delete, max_list, select, zero, one, minusone, add, minus, my_succ, my_prev, same_ptr, maxnum, minnum, gt_list, lt_list, ord_union, insert, nullptr, empty, add]), !.
