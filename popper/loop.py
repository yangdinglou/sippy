...
def popper(settings):
    ...
    if not settings.single_solve:
        ...
        settings.logger.debug(f'Update at line {frameinfo.lineno} in {frameinfo.filename}')
        with settings.stats.duration('init'):
            generator.update_solver(all_handles, bad_handles, all_ground_cons)
        settings.logger.debug(f'Update done at line {frameinfo.lineno} in {frameinfo.filename}')
    ...
    with generator.solver.solve(yield_ = True) as handle:
        ...
        while True:
            ...
            # GENERATE A PROGRAM
            frameinfo = getframeinfo(currentframe())
            settings.logger.debug(f'solve at line {frameinfo.lineno} in {frameinfo.filename}')
            ...
            frameinfo = getframeinfo(currentframe())
            settings.logger.debug(f'After test at line {frameinfo.lineno} in {frameinfo.filename}')
            # FIND MUCS
            ...
            frameinfo = getframeinfo(currentframe())
            settings.logger.debug(f'Constrain at line {frameinfo.lineno} in {frameinfo.filename}')
            # CONSTRAIN
