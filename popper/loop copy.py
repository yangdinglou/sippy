        ...
        with generator.solver.solve(yield_ = True) as handle:
            ...
            while True:
                ...
                if not inconsistent and settings.functional_test and num_pos_covered > 0 and tester.is_non_functional(prog):
                    ...
                    settings.logger.debug('Program is not functional, addgen')
                    add_gen = True
                    ...
                    if not add_gen or len(prog) > 1:
                        for rule in prog:
                            if tester.has_redundant_literal([rule]):
                                settings.logger.debug('Program has redundant literals, addgen')
                                add_gen = True
                                if len(prog) > 1:
                ...
                if settings.recursion_enabled and len(prog) > 2 and tester.has_redundant_rule(prog):
                    settings.logger.debug('Program has redundant rules, addgen')
                    add_gen = True
                ...
                if not inconsistent and num_pos_covered == num_pos:
                    if settings.threshold == 0:
                        ...
                        settings.logger.debug('Found non-separable program, addgen')
                        add_gen = True

                # BUILD CONSTRAINTS
                if add_spec and not pruned_sub_incomplete:
                    settings.logger.debug('Adding specialisation constraint')
                ...
                if add_redund2 and not pruned_sub_incomplete:
                    settings.logger.debug('Adding redundancy constraint2')
