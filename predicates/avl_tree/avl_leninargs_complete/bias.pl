% p(A,B,H):- nullptr(A),zero(C),my_prev(C,B), empty(H) !.
% p(A,B,H):- left(A,E),right(A,C),p(C,F,I),p(E,G,J),maxnum(G,F,D),my_succ(D,B),diff_lessthanone(G,F), value, gt, lt, union, insert !.
...
% 	not body_literal(T, p, _, (_, X, _)),
% 	not head_literal(T, p, _, (_, X, _)).

% :-
% 	body_literal(T, diff_lessthanone, _, (X, Y)),
% 	not body_literal(T, p, _, (_, Y, _)),
% 	not head_literal(T, p, _, (_, Y, _)).
