...
:-
	#count{A,Vars : body_literal(3,inv1,A,Vars)} != 2.
...
input_pointer(tree_root,pointer).

inner_pointer(child,pointer).
inner_pointer(sibling,pointer).
inner_pointer(key,integer).
