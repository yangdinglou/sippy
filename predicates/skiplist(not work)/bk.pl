...
eval_head(Ex, Inlist, 0, 8, Inlist) :- functor(Ex, P, _), call(Ex), member(P,[equal, diff_lessthanone, min_list, delete, max_list, select, zero, one, add, minus, my_succ, my_prev, maxnum, minnum, gt_list, lt_list, ord_union, insert]), !.
...
scores(Ex, FactNum, FactV, SpecV) :- eval_head(Ex, [], AllFactV, SpecV, Outlist), length(Outlist, Callnum), list_to_set(Outlist, Outset), length(Outset, FactNum), FactV is AllFactV/Callnum.
