  ...
  //@ ensures true;
{
  ...
  ret = remove(root0, 0);
  assert(!contains(ret, 0));
  BNode * root1 = build_graph();
  ret = remove(root1, 1);
  assert(!contains(ret, 1));
  BNode * root2 = build_graph();
  ret = remove(root2, 2);
  assert(!contains(ret, 2));
  BNode * root3 = build_graph();
  ret = remove(root3, 3);
  assert(!contains(ret, 3));
  BNode * root4 = build_graph();
  ret = remove(root4, 4);
  assert(!contains(ret, 4));
  BNode * root5 = build_graph();
  ret = remove(root5, 5);
  assert(!contains(ret, 5));
  BNode * root6 = build_graph();
  ret = remove(root6, 6);
  assert(!contains(ret, 6));
  BNode * root7 = build_graph();
  ret = remove(root7, 7);
  assert(!contains(ret, 7));
  BNode * root8 = build_graph();
  ret = remove(root8, 8);
  assert(!contains(ret, 8));
  BNode * root9 = build_graph();
  ret = remove(root9, 9);
  assert(!contains(ret, 9));
  BNode * root10 = build_graph();
  ret = remove(root10, 10);
  assert(!contains(ret, 10));
